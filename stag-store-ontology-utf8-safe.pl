#!/opt/perlbrew/perls/perl-5.14.4/bin/perl -w

eval 'exec /opt/perlbrew/perls/perl-5.14.4/bin/perl -w -S $0 ${1+"$@"}'
    if 0; # not running under some shell

#
# MODIFIED VERSION OF stag-storenode.pl
# https://metacpan.org/pod/DBIx::DBStag#COMMAND-LINE-SCRIPTS
#
# which handles utf-8 encoded term names, definitions and synonyms appropriately
#
    ...
    if (@units) {
        ...
        $H->catch_end_sub(sub {
                              ...
                              if ($handler->depth == 1 &&
                                  ...
                                  # clean up the UTF-8
                                  if ($stag->name eq 'cvterm') {
                                      # warn "processing cvterm: ".$stag->get('name')."\n";
                                      $stag->set('name', encode("UTF-8", $stag->get('name')));
                                      $stag->set('definition', encode("UTF-8", $stag->get('definition')));
                                      map { $_->set('synonym', encode("UTF-8", $_->get('synonym'))) } $stag->find('cvtermsynonym');
                                  }
                                  store($handler,$stag);
                                  return;
                              }
...
#    Data::Stag->parse(-format=>$parser,-fh=>$fh, -handler=>$H);
#    my $str = read_file($fn);
#    $str = encode("UTF8", $str);
#    Data::Stag->parsestr(-str=>$str, -format=>$parser, -handler=>$H);
